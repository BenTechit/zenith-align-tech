...
const TEAM_SIZES = ["1–5 employees", "6–15 employees", "16–30 employees", "31–50 employees", "50+ employees"];
const DEVICE_TYPES = ["Laptop", "Desktop PC", "MacBook / iMac", "Tablet", "Phone / Smartphone", "Other"];
...
const ContactFormSection = () => {
  ...
  const toggleService = (s: string) =>
    setBizServices(prev => prev.includes(s) ? prev.filter(x => x !== s) : [...prev, s]);

  const validateBiz = () => {
    const e: Record<string, string> = {};
    if (!biz.name.trim()) e.name = "Please enter your name";
    if (!biz.business.trim()) e.business = "Please enter your business name";
    if (!biz.location.trim()) e.location = "Please enter your city";
    if (!biz.teamSize) e.teamSize = "Please select your team size";
    if (bizServices.length === 0) e.services = "Please select at least one service";
    if (!biz.phone.trim()) e.phone = "Please enter your phone number";
    setBizErrors(e);
  ...
  const validatePriv = () => {
    const e: Record<string, string> = {};
    if (!priv.name.trim()) e.name = "Please enter your name";
    if (!priv.device) e.device = "Please select a device type";
    if (priv.issue.trim().length < 5) e.issue = "Please describe the issue";
    if (!priv.phone.trim()) e.phone = "Please enter your phone number";
    setPrivErrors(e);
    return Object.keys(e).length === 0;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = mode === "business"
        ? { type: "business", name: biz.name.trim(), business: biz.business.trim(), location: biz.location.trim(), teamSize: biz.teamSize, services: bizServices.join(", "), phone: biz.phone.trim(), email: biz.email.trim(), message: biz.message.trim() }
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-border p-6 md:p-8 space-y-4">
                ...
                <div>
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Name *" value={biz.name} onChange={e => setBiz({ ...biz, name: e.target.value })} className={`${inputClass} pl-10`} maxLength={100} />
                  </div>
                ...
                <div>
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Business Name *" value={biz.business} onChange={e => setBiz({ ...biz, business: e.target.value })} className={`${inputClass} pl-10`} maxLength={100} />
                  </div>
                  {bizErrors.business && <p className="text-destructive text-xs mt-1 ml-1">{bizErrors.business}</p>}
                </div>

                {/* Location + Team size — 2 col */}
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <div className="relative">
                      <MapPin className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground/50" />
                      <input type="text" placeholder="City *" value={biz.location} onChange={e => setBiz({ ...biz, location: e.target.value })} className={`${inputClass} pl-10`} maxLength={80} />
                    </div>
                ...
                <div>
                  <p className="text-xs font-medium text-muted-foreground mb-2 ml-1">Service Required * (select all that apply)</p>
                  <div className="space-y-2">
                    {SERVICE_OPTIONS.map(s => (
                      <label key={s} className="flex items-center gap-3 p-2 rounded-lg hover:bg-accent cursor-pointer transition-colors">
                        <input
                          type="checkbox"
                          checked={bizServices.includes(s)}
                ...
                <div>
                  <div className="relative">
                    ...
                    <input type="tel" placeholder="Phone *" value={biz.phone} onChange={e => setBiz({ ...biz, phone: e.target.value })} className={`${inputClass} pl-10`} maxLength={20} />
                  </div>
                ...
                <div className="relative">
                  ...
                  <input type="email" placeholder="Email" value={biz.email} onChange={e => setBiz({ ...biz, email: e.target.value })} className={`${inputClass} pl-10`} maxLength={100} />
                </div>

                {/* Notes */}
                <div className="relative">
                  <MessageSquare className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground/50" />
                  <textarea placeholder="Additional notes (optional)" value={biz.message} onChange={e => setBiz({ ...biz, message: e.target.value })} rows={3} className={`${inputClass} pl-10 resize-none`} maxLength={1000} />
                </div>
                ...
                {status === "error" && <p className="text-center text-xs text-destructive">Something went wrong. Please try again or call directly.</p>}
              </form>
            ) : (
              /* Private form */
              <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-border p-6 md:p-8 space-y-4">
                ...
                <div>
                  <div className="relative">
                    <User className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground/50" />
                    <input type="text" placeholder="Name *" value={priv.name} onChange={e => setPriv({ ...priv, name: e.target.value })} className={`${inputClass} pl-10`} maxLength={100} />
                  </div>
                ...
                <div>
                  <div className="relative">
                    <MessageSquare className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground/50" />
                    <textarea placeholder="Describe the issue *" value={priv.issue} onChange={e => setPriv({ ...priv, issue: e.target.value })} rows={4} className={`${inputClass} pl-10 resize-none`} maxLength={1000} />
                  </div>
                ...
                <div>
                  <div className="relative">
                    <Phone className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground/50" />
                    <input type="tel" placeholder="Phone *" value={priv.phone} onChange={e => setPriv({ ...priv, phone: e.target.value })} className={`${inputClass} pl-10`} maxLength={20} />
                  </div>
                ...
                <div className="relative">
                  <Mail className="absolute left-3 top-3.5 w-4 h-4 text-muted-foreground/50" />
                  <input type="email" placeholder="Email" value={priv.email} onChange={e => setPriv({ ...priv, email: e.target.value })} className={`${inputClass} pl-10`} maxLength={100} />
                </div>
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex items-start gap-3">
                ...
                <a href="https://wa.me/972544991540" target="_blank" rel="noopener noreferrer" className="text-foreground hover:text-primary transition-colors font-medium">WhatsApp</a>
              </div>
            </div>
